...
安装后在模块的 Env 区域填写参数。界面中的 token 和 SSID 示例只是输入提示，必须替换为实际配置；`cache_diff` 默认开启，`cache_max_age_hours` 默认 `6`。需要立即更新时，在 Egern 脚本列表中运行“PO0 手动更新”；“PO0 状态”Widget 只负责读取并展示最近结果，不会自动调用 API。
