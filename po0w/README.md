...
- 自动更新会先通过 `DIRECT` 查询公网 IP；默认仅比较当前 slot，也可配置为遍历该 token 缓存中的所有 slot，只要公网 IP 已存在于白名单便不调用写入 API。客户端缓存默认 6 小时过期。
...
安装后在模块的 Env 区域填写参数。界面中的 token 和 SSID 示例只是输入提示，必须替换为实际配置；`cache_diff` 和 `cache_diff_only_current_slot` 默认开启，`cache_max_age_hours` 默认 `6`。需要立即更新时，在 Egern 脚本列表中运行“PO0 手动更新”；“PO0 状态”Widget 只负责读取并展示最近结果，不会自动调用 API。
...
Stash 覆写没有交互式参数表。安装前或导入后，请同时编辑 `cron.script` 与 `tiles` 中的 `argument`，将示例 token、SSID、`cache_diff`、`cache_diff_only_current_slot` 和 `cache_max_age_hours` 替换为实际配置，并确保两处参数完全一致。支持 `$trigger` 的版本会让 Tile 的非按钮刷新只读取最近结果，点击刷新按钮时按当前网络选择 token、忽略 `skip_ssids` 并立即更新；如果运行环境没有提供 `$trigger`，Tile 执行会安全降级为手动更新。
...
安装后在插件参数中填写 `host`、`cellular_tokens`、`wifi_tokens`、`skip_ssids`、`cache_diff`、`cache_diff_only_current_slot` 和 `cache_max_age_hours`。Loon 的“PO0W 手动更新”会忽略 `skip_ssids` 并立即调用 API；“PO0W 查看最近结果”只读取共享快照，通过通知展示上一次请求的网络环境、公网 IP、运营商、当前 IP、白名单和每个请求的结果。
...
将文件中的 `[task_local]` 内容合并到 Quantumult X 配置。Quantumult X 没有交互式参数表，请同步编辑四条任务 URL 的 `#` fragment，将示例 token、SSID、`cache_diff`、`cache_diff_only_current_slot` 和 `cache_max_age_hours` 替换为实际配置并保持四处参数一致；参数包含 `&`、`#` 等 URL 保留字符时需要先编码。
...
| `cache_diff` | 否 | `true` 时查询 token 白名单缓存后决定是否写入；`false` 时直接调用白名单 API。默认 `true` |
| `cache_diff_only_current_slot` | 否 | `true` 时仅比较当前配置的 slot；`false` 时比较该 token 缓存中的所有 slot，只要公网 IP 存在于任意 `/24` 网段便跳过写入。默认 `true` |
...
cache_diff=true
cache_diff_only_current_slot=true
...
各客户端的自动任务会先通过 `126`、`BILI`、`IPIP` 三个来源依次查询公网 IP，所有查询同样强制使用 `DIRECT`。取得公网 IP 后，脚本按当前 `token` 找到共享的完整白名单缓存。`cache_diff_only_current_slot=true` 时只读取当前配置 slot 的 `/24` 网段，命中后显示“未请求（IP 未变化）”；设为 `false` 时遍历该 token 的所有 slot，只要公网 IP 命中任意网段便显示“未请求（IP 已存在）”。两种模式命中缓存后都不会调用白名单写入 API。缓存时间缺失、系统时间倒退或缓存超过 `cache_max_age_hours` 时，会忽略网段比较并强制调用白名单 API。公网 IP 查询失败时也会继续调用写入 API，避免第三方查询服务异常造成漏更新。

`cache_diff` 只控制是否执行上述缓存比较。设为 `false` 时，网络变化和定时任务仍会查询并展示公网 IP 与运营商，但会跳过比较步骤，直接调用所有有效 token 的白名单写入 API。`cache_diff_only_current_slot` 仅在 `cache_diff=true` 时生效，并且每个 token 只检查自己的白名单缓存，不会跨 token 比较。`cache_max_age_hours` 只在 `cache_diff=true` 的自动任务中生效；手动刷新本身就是强制更新，不受缓存范围和缓存时间影响。
...
`token-cache` 使用 `token` 哈希定位，相同 token 即使配置不同 slot 也共享同一份完整白名单缓存。比较范围由 `cache_diff_only_current_slot` 决定；缓存不区分蜂窝和非蜂窝，也不保存原始 token。每个条目的 `updatedAt` 只在 API 成功返回有效 `whitelist` 后刷新，缓存命中、403 或失败结果不会延长有效期。用于隔离不同模块配置的存储键只包含配置哈希。旧的单 IP 缓存不会继续使用，新的快照也不再保存 `lastRequestResults` 和 `lastUsableResults`。
